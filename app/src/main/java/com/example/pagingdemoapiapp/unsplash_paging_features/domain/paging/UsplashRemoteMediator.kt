package com.example.pagingdemoapiapp.unsplash_paging_features.domain.paging
...
class UnsplashRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            val currentPage = when(loadType) {
                LoadType.REFRESH -> {
                    val remoteKeys = getRemoteKeyClosestPosition(state)
            ...
            unsplashDatabase.withTransaction {
                ...
                unsplashImageDao.insertImages(images = response)
            }
    ...
    private suspend  fun getRemoteKeyClosestPosition(
        state: PagingState<Int, UnsplashImage>
    ): UnsplashRemoteKeys? {
        return state.anchorPosition?.let{ position ->
            state.closestItemToPosition(position)?.id?.let{ id ->
            unsplashRemoteKeysDao.insertRemoteKeys(id = id)
            }
        }
    }
    ...
    private suspend  fun getRemoteKeyForFirstItem(
        ...
        return state.pages.firstOrNull { it.data.isNotEmpty() } ?.data?.firstOrNull()
            ?.let { unsplashImages ->
                unsplashRemoteKeysDao.insertRemoteKeys(id = unsplashImages.id)
            }
    }
